...
public class GameField_Controller extends TemplateController{
		...
		if (this.game.getTurn().getName().equals(this.player.getName())){
			...
			case Buy:
				if(this.game.getHandCards(player).checkForMoneyCard()==true || game.getAdditionalMoney(player)>0){
					...
					System.out.println("KAUFPHASE HAT BEGONNEN --> bitte wähle eine oder mehrere Karten");
				}else{
					model.sendNoMoneyCardMessage();
					System.out.println("Kaufphase wurde übersprungen, weil keine Geld-Karte auf der Hand");
				}
				break;
			...
			case actionMine:
				...
				if(this.game.getHandCards(player).checkForMoneyCard()==true) {
					view.getPlayPhase().setText("Karte Mine ausführen");
					}else{
			...
			case actionUmbau:
					view.getPlayPhase().setText("Karte Umbau ausführen");
				break;
			...
			case actionWerkstatt:
					view.getPlayPhase().setText("Karte auswählen");
				break;
				
			case actionKapelle:
				view.getPlayPhase().setText("Karte Kapelle ausführen");
				break;
	...
	//füht die aktuelle Phase aus sobald der Button playPhase gedrückt wird
	public void playDraw() throws IOException{
		...
		case Buy:
			...
			System.out.println("Wert der ausgewählten Karten: "+value);
	...
	public void updateView(){
		...
		view.getLblAdditionalMoney().setText("\t Zusätzliche Geldeinheiten: " + game.getAdditionalMoney(player));
		view.getLblPlayer().setText("\t " + player.getName());
		view.getLblAdditionalBuys().setText("\t Zusätzliche Kaufeinheiten: " + (game.getBuyCount(player)-1));
