		...
		messageList.messages.addListener((ListChangeListener) (event -> {
			...
			if (incomingMessage.getMessageType() == EnumMessageType.cardsetMismatchMessage) {				
				...
				if (player.getLanguage() == EnumLanguage.English) {
					...
					a7.setTitle("Kartensets stimmen nicht überein");
			...
			if (incomingMessage.getMessageType() == EnumMessageType.customCardSetMismatchMessage) {				
				...
				if (player.getLanguage() == EnumLanguage.English) {
					...
					a7.setTitle("Kartensets stimmen nicht überein");
					a7.setContentText("Du hast das Kartenset Custom mit den folgenden Karten gewählt: \n" + mycardset + "\nDein Gegner hat das Kartenset Custom mit folgenden Karten gewählt: \n" + opponentcardset + "\nEin Spiel ist nur mit übereinstimmenden Sets möglich!!");
