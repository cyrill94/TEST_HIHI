...
public class ServerController{
	...
	messageListServer.addListener((ListChangeListener) (event -> {
		...
		else if (incomingMessage.getMessageType() == EnumMessageType.settingsCustomCards) {
			SettingsCustomCardSetsMessage settingscustomcardsetmessage = (SettingsCustomCardSetsMessage) incomingMessage;
			serverModel.setCustomCardSet(settingscustomcardsetmessage.getPlayer(), settingscustomcardsetmessage.getCardStrings());


			for (Slot slot : slotList) {
				if (incomingMessage.getSocketAddress().equals(slot.getClientSocketAddress())) {
					slot.setPlayer(settingscustomcardsetmessage.getPlayer());
			    }
			}
		}
