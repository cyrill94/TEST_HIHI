...
public class Game implements Serializable {
	...
	public Game(ArrayList<String> cardStrings, Player player1, Player player2){
		this.cardSet = new ArrayList<ActionCard>();
		this.ActionStacks = new ArrayList<Stack>();
		this.StandardStacks = new ArrayList<Stack>();
		this.spellCardStack = new Stack<SpellCard>();
		...
		initializeCustomCardSet(cardStrings);;
		initializeStacks();
		initializeDeck(gamePlayer1);
		initializeDeck(gamePlayer2);
		System.out.println("deckstack initialisiert player1: " + this.gamePlayer1.getdeckStack());
	...
	public Card drawCardFromDeck(Player player) {
		if (player.equals(gamePlayer1.getPlayer())) {	
			...
			Card c = gamePlayer1.drawCardFromDeck();
			gamePlayer1.addCardToHand(c);
			return c;
		}
		else {
			...
			Card c = gamePlayer2.drawCardFromDeck();
			gamePlayer2.addCardToHand(c);
			return c;
		}
	}
...
/** @author Sven und Thomas **/
	...
	public int getDeckSize(Player player) {
		if (player.getName().equals(gamePlayer1.getPlayer().getName())) {	
			return gamePlayer1.getdeckStack().getSize();
		}
		else {
			return gamePlayer2.getdeckStack().getSize();
		}
	}
	
	/** @author Thomas **/
	public int getGraveyardSize(Player player) {
		if (player.getName().equals(gamePlayer1.getPlayer().getName())) {	
			return gamePlayer1.getgraveyardCards().getSize();
		}
		else {
			return gamePlayer2.getgraveyardCards().getSize();
		}
	}
	
	/** @author Thomas **/
	public Card getTopGraveyardCard(Player player) {
		if (player.getName().equals(gamePlayer1.getPlayer().getName())) {	
			return gamePlayer1.getgraveyardCards().peekCard();
		}
		else {
			return gamePlayer2.getgraveyardCards().peekCard();
		}
	}
